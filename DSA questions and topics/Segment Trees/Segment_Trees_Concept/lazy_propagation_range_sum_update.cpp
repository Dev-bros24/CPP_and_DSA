...
public:
    ...
    void update(int ind, int low, int high, int l, int r, int value){ // Range update, updating each value in the range by +value
        ...
        // this node if any remaining and propagate downwards
        if(lazy[ind] != 0){
            seg[ind] += (high-low+1) * lazy[ind];
            if(low != high){ // propagate the lazy update downwards if it's not a leaf node
...
void solve(){
    ...
    while(t--){
        ...
        if(type == 1){
            ...
            cin>>l>>r;
            cout<<"Sum in the range "<<l<<" to "<<r<<" is: "<<sg.query(0, 0, n-1, l, r);
